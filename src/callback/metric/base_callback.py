...
class BaseMetricCallback(Callback):
    ...
    def setup(
        ...
        for label_name in label_names:
            self._train_metric_dict[f"train/{self._logging_name}/{label_name}"] = (
                self._metric.clone()
            )
            self._valid_metric_dict[f"valid/{self._logging_name}/{label_name}"] = (
                self._metric.clone()
            )
            self._test_metric_dict[f"test/{self._logging_name}/{label_name}"] = (
                self._metric.clone()
            )

        pl_module.register_module(
            f"train/{self._logging_name}", self._train_metric_dict
        )
        pl_module.register_module(
            f"valid/{self._logging_name}", self._valid_metric_dict
        )
    ...
    def _align_pred_label_list(
        ...
        for i in range(len(label_names)):
            aligned_list.append((label_names[i], pred[:, i], label[:, i]))
    ...
    def on_train_batch_end(
        ...
        pl_module: pl.LightningModule,
        outputs: dict[str, torch.Tensor],
        # outputs - ChalearnLightningModule return values
        batch: Any,
        batch_idx: int,
